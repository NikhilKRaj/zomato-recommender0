...
if (apiUrl) {
  ...
  console.error(
    "[vercel] ERROR: RAILWAY_API_URL is not set. Filter dropdowns will not work in production.",
  );
  process.exit(1);
} else {
  console.warn(
    "[vercel] RAILWAY_API_URL is not set; skipping /api proxy rewrite (local build).",
  );
}
